...
from forms2 import LoginForm, RegisterForm
import psycopg2
import requests
import datetime
import pytz
...
token = '5ec6f4f6fcb1ee4e5a917dea37a1c02acd4ead36'
...
def check_branch():
    ...
    flash('No new branches were detected!', 'danger')
    return redirect(url_for('leaderboard'))
...
def login():
    ...
    if user is not None:
        flash(f'You are already logged in as github user {user}', 'danger')
        return redirect(url_for('leaderboard'))
    ...
    if request.method == 'POST':
        register = register_form.validate_on_submit()
        login = login_form.validate_on_submit()
